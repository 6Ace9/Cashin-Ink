# booking.py  ← FINAL VERSION – LOGO & BACKGROUND 100% VISIBLE
...
# ============== FORCE IMAGES TO LOAD (WORKS EVERYWHERE) ==============
...
bg_b64 = load_image_b64("background.png")

# ============== CSS + LOGO + BACKGROUND ==============
logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="display:block; margin:0 auto; width:320px; filter:drop-shadow(0 0 20px #00C853);">' if logo_b64 else "<h1 style='color:#00C853;text-align:center;'>CASHIN INK</h1>"

bg_css = f"background: linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.85)), url('data:image/png;base64,{bg_b64}') center/cover no-repeat fixed;" if bg_b64 else "background:#000;"

st.markdown(f"""
<style>
    .stApp {{ {bg_css} color:white; }}
    .main {{ background:rgba(0,0,0,0.5); padding:30px; border-radius:20px; max-width:900px; margin:20px auto; border:1px solid #00C85333; }}
...
# ============== EVERYTHING ELSE (100% SAME AS BEFORE) ==============
...
with st.form("booking_form"):
    ...
    if submit:
        if not all([name, phone, email, description]) or age < 18 or not agree:
            st.error("Fill all fields & agree to deposit")
        else:
            ...
            if conflict:
                st.error(f"Slot just taken by {conflict[0]}")
                st.stop()

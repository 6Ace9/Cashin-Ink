# app.py ← FINAL: FULLY CUSTOM DARK DATE & TIME PICKERS (NO WHITE EVER)
...
if "selected_date" not in st.session_state: st.session_state.selected_date = datetime.today() + timedelta(days=1)
if "selected_time" not in st.session_state: st.session_state.selected_time = "13:00"
...
with st.form("booking_form"):
    ...
    with tc:
        ...
        time_options = [f"{h:02d}:00" for h in range(12, 21)]
        time = st.selectbox("", time_options, index=time_options.index(st.session_state.selected_time) if st.session_state.selected_time in time_options else 1, label_visibility="collapsed")
    ...
    if submitted:
        if not all([name, phone, email, description]) or age < 18 or not agree:
            ...
            start_dt = STUDIO_TZ.localize(datetime.combine(date, appt_time))
            ...
            c.execute("INSERT INTO bookings VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
                bid, name, age, phone, email, description, str(date),
                datetime.strptime(time, "%H:%M").strftime("%-I:%M %p"),
